...
## 6. اختبار إضافة مهمة من واتساب | WhatsApp Personal Intake

1. حدّث متغير `WHATSAPP_TASK_LISTS` ليتضمن قائمة نشطة (أو استخدم `CLICKUP_SAMPLE_LIST_ID`).
2. أرسل رسالة من حساب واتساب أحد أعضاء الفريق (الموجود في `src/config/team.js`). مثال: `عمل عرض سعر لونا غداً`.
3. تأكد من وصول رد تلقائي يؤكد فهم الطلب ثم ملخص إنجليزي يتضمن العنوان والقائمة المقترحة.
4. جرّب الرد بـ `نعم` ثم أرسل صورة أو ملف، وتأكد من ظهور رسالة `تم حفظ المرفق`.
5. أرسل `تم` للتحويل النهائي، ثم افتح ClickUp للتحقق من ظهور المهمة بالرابط المرسل في واتساب.
6. اختبر الأوامر الجانبية:
   - `إلغاء` يجب أن يمسح الجلسة بدون إنشاء مهمة.
   - `جديد` يعيد البدء فوراً.

> إذا لم تصلك الرسائل تأكد من ظهور `WhatsApp personal task intake ready` في الطرفية ومن أن رقمك مدرج ضمن أعضاء الفريق.
