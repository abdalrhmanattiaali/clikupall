...
### 5. Real Data Sources | مصادر بيانات حقيقية
- يجمع البيانات من **ClickUp** مباشرةً لكل عضو عبر خدمة `clickupService`
- يضيف الأرقام اليومية من مستودع الإنتاجية `productivityRepository` لضمان دقة "المهام المكتملة اليوم"
- يعتمد على ملف الفريق `config/team.js` لتحديد الأعضاء المُراقَبين وأسمائهم في الرسائل
- يطابق المعرفات المتعددة (الاسم، البريد، رقم ClickUp) لكل عضو حتى تبقى الإحصائيات منسجمة عند تغير بيانات ClickUp

### 6. Reliability & Fallbacks | الاعتمادية وخطط الطوارئ
- يسجّل آخر 50 رسالة لتجنب التكرار (المصفوفة `contentHistory` مع ضبط الحجم التلقائي)
- يعيد المحاولة باستخدام رسائل جاهزة (fallback) في حال فشل الذكاء الاصطناعي أو تعطل الاتصال
- يراقب حالة واتساب قبل الإرسال (`whatsappService.isClientReady`) لتفادي فقدان الرسائل ويُسجّل النتيجة في السجلات
- كل محاولة توليد أو إرسال تُوثّق برسائل `logger.success / logger.error` مع تفاصيل الوقت والنبرة والمحتوى المختار لتسهيل التتبع

### 7. Delivery Workflow & Testing | مسار الإرسال والاختبار
- يتم تشغيل الإشعارات أوتوماتيكياً عبر المجدول `schedulerService` في أوقات 08:00 / 12:00 / 16:00 / 20:00
- يمكن تشغيل كل فترة يدوياً بواسطة واجهات الاختبار (`/test/motivation/:slot` و`/test/send-motivation/:slot`) للمراجعة قبل الإنتاج
- يدعم وضع الاختبار إرجاع JSON يحتوي على حالة النجاح، طول الرسالة، ونوع الموفر الذكي لتأكيد صحة الاستجابة
- نفس المنهجية تُستَخدَم أثناء تطوير قوالب جديدة أو ضبط النبرة لأن المطوّر يستطيع مراقبة البيانات والمخرجات دون إرسال فعلي للمجموعة
